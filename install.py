...
LIBTORCH_URL = {
    ("Linux", "x86_64", "cu128"): "https://download.pytorch.org/libtorch/cu128/libtorch-shared-with-deps-2.8.0%2Bcu128.zip",
    ("Windows", "x86_64", "cu128"): "https://download.pytorch.org/libtorch/cu128/libtorch-win-shared-with-deps-2.8.0%2Bcu128.zip",
}
...
def download_file(url: str, dst: Path) -> None:
    ...
    print(f"       to: {dst}")
    req = Request(url, headers={"User-Agent": "elastix-impact-installer/1.0"})
    with urlopen(req) as r, open(dst, "wb") as f:
        ...
        while True:
            ...
            read += len(b)
            if total_i:
                pct = (read * 100) // total_i
                print(f"\r  ... {pct}% ({read/1e6:.1f}MB/{total_i/1e6:.1f}MB)", end="")
        print()
...
def main() -> int:
    ...
    group.add_argument(
        "--force-cpu",
        action="store_true",
        help="Force CPU install even if NVIDIA present"
    )
    group.add_argument(
        "--force-cuda",
        action="store_true",
        help="Force CUDA install (fails if no suitable driver)"
    )
    args = ap.parse_args()
    ...
    if os_name not in ("Linux", "Windows", "Darwin"):
        ...
        return 2
    if arch != "x86_64":
        print(f"Unsupported arch: {arch} (expected x86_64)")
        return 2
    
    flavor = "cpu"
    if args.force_cuda:
        if not has_nvidia or not driver_ok_for_cuda(os_name, drv):
            print("CUDA forced but NVIDIA driver/GPU not suitable.")
            print(f"Detected: has_nvidia={has_nvidia}, driver={drv}")
            return 3
        flavor = "cu128"
    ...
    print(f"System: {os_name} {arch}")
    print(f"NVIDIA: {has_nvidia}, driver={drv}")
    print(f"Selected flavor: {flavor}")

    key = (os_name, arch, flavor)
    if key not in ELX_ASSET_TEMPLATE:
        print(f"No elastix asset configured for {key}")
        return 2
    if flavor != "cpu" and key not in LIBTORCH_URL:
        print(f"No libtorch url configured for {key}")
        return 2
